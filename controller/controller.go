...
func (c Controller) InitRouters() {
	...
	slack.POST("/tbb_sethours", func(gCtx *gin.Context) {
		...
		//TODO We have to find your right to PARTY! Sorry. I need to find a way to work gather POST values. Because of GIN integration MultipartForm() not accessible anymore.
		UserID := gCtx.PostForm("user_id")
	...
	slack.POST("/tbb_removetask", func(gCtx *gin.Context) {
		...
		// Unfound
		task, err := c.app.GetTask(cmdText); c.checkDBError(err, fmt.Sprintf("При поиске задачи «%s» в базе данных произошла ошибка:\n%v", cmdText), gCtx)
		...
		rowsAffected, childRowsAffected, err := c.app.RemoveTaskByIdAndChildHours(task.ID); c.checkDBError(err, fmt.Sprintf("При удалении задачи «%s» из базы данных произошла ошибка:\n%v", cmdText), gCtx)
		...
		if rowsAffected == 1 {
			...
			currentTask, err := c.app.GetCurrentTask(); c.checkDBError( err, "При запросе текущей задачи голосованияиз базы данных, произошла ошибка", gCtx )
